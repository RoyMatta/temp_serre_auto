...
T_Bouton lBoutons[]=
  {
    {false, false, BTN_ArretUrgence, 0},
...
void activeMoteur(int numero, int etat) { //demarrage du moteur
  ...
  if(etat == MOTEUR_MONTE) {
    digitalWrite(lMoteurs[numero].dir, LOW);    //REMPLACER LOW par sens_montee et définir sens_montée
  } else {
...
void check_moteur(int numero) {   //TBD ajouter variation d'un timeout pour mode manu, ajouter argument timeout
  ...
  if(mode_moteur_auto) {
    duree_moteur = rapport_ouverture*duree_moteur*0.01;   ///ajuste duree ouvrant moteur
  }
...
bool changeEtatBouton(int numero) { //seulement pour boutons du mcp, pas pour bouton arret urgence
  ...
  if(lBoutons[numero].timestamp==0) {
    lBoutons[numero].timestamp = millis();
  }
...
void loop_boutons() {
  ...
  if(changeEtatBouton(BTN_PUSH_ON)) {     //Lecture avec ROY arreté là   // il faut mettre boutonPresse(BTN_PUSH_ON)
    syst_on = lBoutons[BTN_PUSH_ON].etat;
...
void setup() {
  ...
  pinMode(MOTEUR_A_FC_HAUT, INPUT_PULLDOWN);  //capteurs de fin course TBD vérifier dans test si ! ou non avec pulldown
